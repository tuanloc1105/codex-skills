...
export function databaseError(error, credential, code = 'DATABASE_ERROR') {
  if (error instanceof DataDebugError) return error;
  return new DataDebugError(code, redactMessage(error, [credential?.secret]));
}
...
export function mutationDatabaseError(error, credential, outcomeMayBeUnknown) {
  ...
  if (error instanceof DataDebugError && error.code === 'MUTATION_OUTCOME_UNKNOWN') return error;
  return new DataDebugError(
...
export function verifyPlannedIdentity(expected, actual) {
  ...
  const identityKeys = ['database', 'principal', 'serverIdentity', 'clusterSystemIdentifier'];
  if (Object.hasOwn(expected, 'instanceRunId')) identityKeys.push('instanceRunId');
