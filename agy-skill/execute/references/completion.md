...
- Allow `simplify` to apply verified in-scope fixes (`VERIFIED`). For `NEEDS_DECISION` proposals whose tradeoff or behavior cannot be inferred safely, ask the user using `ask_question` before applying them; drop `REJECTED` proposals without editing.
...
- The current execution session's commit range or remaining working-tree diff includes durable agent-facing changes, such as new or changed project structure, package boundaries, entrypoints, scripts, commands, workflows, tests, generated assets, configuration, deployment steps, migrations, or repo conventions (`AGENTS.md`, `GEMINI.md`, rules).
...
At the end, ask the user whether they want a security review of the current execution session's changes using `ask_question` (or chat fallback).
...
Then ask whether the user wants `security-review` on the current execution session's changes when implementation reached `Implemented`, using `ask_question` (or chat fallback), unless they already answered that question in the current turn.
