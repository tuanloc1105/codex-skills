...
- **Coordinator Execution Location**: The coordinator agent itself must run all coordinator-owned implementation edits, checks, staging, commits, integration, and simplify-driven fixes inside that dedicated worktree (e.g. running `run_command` with `Cwd: <dedicated-worktree-path>` and targeting file edits with `replace_file_content` / `write_to_file` inside that dedicated worktree). The adopted execution record remains at its exact bound path and is the only permitted execute-mode write outside the dedicated worktree when that path lives elsewhere.
...
2. Follow `Dedicated Worktree`: retain the captured or recorded initial terminal anchor, resolve Git ownership from each implementation target, and create or safely reuse a dedicated worktree for each target repository. Apply ignore setup only when its destination is inside a checkout, persist the repository/worktree mapping, and perform all coordinator and implementation work in that dedicated worktree. Skip worktree setup only for actual non-Git targets, not merely because the terminal starts in an umbrella directory.
3. Inspect enough repository context to execute safely using Antigravity read tools (`view_file`, `grep_search`, `find_by_name`, `list_dir`).
...
6. Mark the selected phase items in progress and dispatch each eligible delegated phase with the required ownership, workspace isolation, and return contract via `invoke_subagent`.
7. Execute any coordinator-owned phase that can run concurrently without conflicting with active subagents inside the dedicated worktree.
8. Collect subagent reports, inspect actual changed files or resources against the baseline and assigned ownership, integrate changes from subagent workspace branches into the dedicated worktree, and review each implementation.
...
13. Use `simplify` to review the complete current-session commit range from the captured starting `HEAD` through the current `HEAD`, together with any remaining in-scope working-tree changes. For non-Git targets, review the complete set of recorded current-session path changes against their saved before-state evidence instead.
14. Fix verified `simplify` findings that are in scope (`VERIFIED`). If `simplify` produces `NEEDS_DECISION` proposals whose tradeoff or behavior cannot be inferred safely, ask the user using `ask_question` before applying them; drop `REJECTED` proposals without editing.
...
16. If the current execution session's commit range, remaining working-tree diff, or recorded non-Git path changes contain substantial agent-facing changes that are not already covered in agent docs, use `update-agent-docs` with the session-change-only scope in this skill.
...
Ask the user (via `ask_question`) only after these recovery paths are exhausted and the Genuine Blocker Definition is satisfied.

## Failure and Conflict Handling

A subagent failure is not automatically a plan blocker. Inspect any partial changes, preserve pre-existing user work, and choose the safest recovery: retry, reassign, finish locally in the dedicated worktree, or serialize the phase. Continue unrelated ready phases when safe and keep dependent phases pending.
