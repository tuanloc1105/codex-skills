...
- **Antigravity Interactive Modal (`ask_question` tool)**:
  - When seeking plan approval or resolving an open design decision, prefer calling the `ask_question` tool.
  - Provide 2-4 concrete options representing user answers (e.g. `(Recommended) Duyệt kế hoạch hiện tại; chưa triển khai`, `Sửa cục bộ một số điểm`, `Đổi hướng tiếp cận khác`).
  - Do not add an explicit "Other" option in tool arguments.
...
- **Chat Fallback & Saved Markdown**:
  ...
  - Accept a bare number such as `1` or a number plus detail.
