...
- Use a literal script input (for example, a quoted heredoc via `run_command` in a compatible shell) so Markdown backticks, dollar signs, and newlines are not evaluated by the shell. Use explicit UTF-8 encoding and preserve existing newline style and file permissions.
...
- Before approval or handoff, verify that all open issues are resolved, no question blocks execution, and for phased plans, phase files, dependencies, waves, Antigravity Workspace settings (`branch` for mutating delegated phases, `share` strictly for read-only delegation, or `N/A` for sequential phases), and ownership are consistent between `plan.md` and phase files.
...
## Question and Open-Issue Contract (Antigravity Interactive & Chat Fallback)
...
- **Antigravity Interactive Modal (`ask_question` tool)**:
  - When seeking plan approval or resolving an open design decision in an interactive session, prefer calling the `ask_question` tool.
  - Provide 2-4 concrete options formatted as direct user responses (e.g. `(Recommended) Duyệt kế hoạch hiện tại; chưa triển khai`, `Sửa cục bộ một số điểm`, `Đổi hướng tiếp cận khác`).
  - Prefix the recommended choice with `(Recommended)`.
  - Do not add an explicit "Other" option in tool arguments (the UI provides a write-in field by default).
  - Preserve the displayed option order when recording the question in `decisions.md`.
- **Chat Fallback & Saved Markdown**:
  - When the interaction channel is chat fallback or when recording questions in saved Markdown, put each option on its own line with an explicit consecutive number: `1.`, `2.`, `3.`, `4.` as needed. Start at `1.`, leave a blank line between the question and its list, and never substitute bullets (`-`, `*`, `•`), checkboxes, letters, inline choices, or repeated `1.` markers. This is a required response format, not merely an example style.
  ...
  - Accept a bare number such as `1` as selection of that option in the pending question, or a number plus detail such as `4. đánh giá lại phương án fix`. Apply any supplied qualification; do not require the user to repeat the option label. A bare selection of `Other` or an option needing a value does not supply the missing detail: ask a focused numbered follow-up.
- **Single Pending Question Guard**:
  - Keep only one user-facing question awaiting an answer at a time so a bare number or selection is unambiguous. A record may retain multiple open questions, each with its own numbered options and stable question ID; present only the next question in chat or via `ask_question`.
- **Preserve Option Mapping**:
  - Preserve the pending question's option mapping (or number-to-option mapping) in the record (`decisions.md` and Active Snapshot) so resumed sessions interpret short replies consistently.
- **Out-of-Range and Revised Mapping Behavior**:
  - If the user's number or response is out of range, ambiguous, or if choices must change, present the revised question before accepting a selection against it; clarify with concrete options instead of guessing.
...
- **Saved Open Question Contract**:
  - Apply these rules to questions in chat or interactive prompt and to every item in the proposed or saved plan's `decisions.md` (or `Open Questions` section).
  - For each open question saved in the record, record its stable question ID, concrete options, recommendation/default when applicable, and whether it blocks execution (`blocks-execution: yes | no`).
- **Validation Before Save and Approval**:
  - Before sending a response or saving a plan, check that every user-facing question and open issue has its own option list and that chat or interactive prompt has only one pending question. Rewrite any bulleted choices before sending.
  - Approval requires that no blocking open question remains unresolved (`blocks-execution: yes` must be 0) and all accepted decisions are recorded in `decisions.md`.
