# Plan Conversation Workflow (Antigravity Edition)
...
3. Gather only the missing information that materially changes the plan. Keep questions concise and prefer the `ask_question` tool; do not ask for details that can be discovered safely from the workspace.
4. Inspect enough context to remove guesswork:
   - Relevant repository instructions (`AGENTS.md`, `GEMINI.md`, rules) and local conventions
...
5. Establish an existing-behavior and regression-safety baseline before proposing changes to an existing mechanism:
   ...
   - Identify the existing tests, checks, logs, or manual reproduction that demonstrate the baseline; run only safe read-only checks and record any checks that could not be run
...
7. Ask the user to approve or revise the plan using `ask_question` (or chat fallback). Present approval, targeted revision, broader rework, and pause/cancel as applicable options. Approval is required before changing the existing draft record to its final execute-ready status.
