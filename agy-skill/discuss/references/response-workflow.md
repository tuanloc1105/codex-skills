# Discuss Response Workflow (Antigravity Edition)
...
11. Otherwise, when the discussion is settled and no blocking question remains, apply `Settled Discussion Transition Gate`: prompt the user via `ask_question` (or numbered chat format) to choose whether they want to create an implementation `plan` or transition directly to `execute`, and wait.
...
18. Format every question that needs a user response using the `ask_question` tool when possible, or as its own consecutively numbered option block under the `Question Style` contract.
