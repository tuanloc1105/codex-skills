#!/usr/bin/env bash
#
# sync-claude-skill.sh — sync the Claude Code skills under claude-skill/ in this
# repository into ~/.claude/skills/<name>/.
#
# Source of truth: <repo>/claude-skill/<name>/   (this repository)
# Destination:     ~/.claude/skills/<name>/
#
# This mirrors the same one-way, source-wins sync pattern used by
# scripts/sync-skills.sh for the top-level Codex skills, but targets Claude
# Code's personal skills directory instead of ~/.codex/skills. The ported
# skills keep their original names, so each one installs as /<name> in Claude
# Code and overwrites any same-named personal skill already there.
#
# Usage:
#   ./scripts/sync-claude-skill.sh              # sync every skill under claude-skill/
#   ./scripts/sync-claude-skill.sh discuss plan # sync only the named skill(s)
#
# Requires: rsync
set -euo pipefail
...
src_root="$repo_root/claude-skill"
dest_root="${CLAUDE_SKILLS_DEST:-$HOME/.claude/skills}"
...
# Discover every skill: a direct child of claude-skill/ containing a SKILL.md.
available_skills=()
