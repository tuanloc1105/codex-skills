...
| `agent-prompt-code-review-part-6-medium-effort-mode.md`; `agent-prompt-code-review-part-7-high-effort-mode.md` | `mode-playbooks.md` — Medium and High | Retains 3+5 angles, caps, and precision/recall split; triggered D/E checks fold into A/C. Local fallback retains a separate same-context self-check instead of dedup-only output. |
...
| `agent-prompt-code-review-part-5-recall-biased-verification-phase.md`; `skill-code-review-phase-2-verify-recall-biased.md` | `verification.md` — Recall Bias | Requires a code-evidenced mechanism and specific unknown premise; non-refutation alone cannot retain speculation. |
...
| `skill-code-review-phase-0-gather-the-diff.md` | `SKILL.md` — Gather the Diff; `target-resolution.md` | Local divergence: merge-destination defaults replace tracking-upstream defaults; explicit unpushed-only retains upstream. Final-state evidence replaces temporal concatenation; exact target types, metadata and commentability remain. |
| `agent-prompt-code-review-part-10-reportfindings-output-format.md`; `skill-code-review-output-findings-json-array.md`; `tool-description-report-code-review-findings.md` | `reporting-and-actions.md` | Ported caps and no-duplication contract. Caller schema wins; tool availability alone does not select typed output. Severity is emitted only when accepted. Canonical four-field JSON carries PLAUSIBLE uncertainty in scenario text; richer certainty/action fields remain opt-in. Truncation disclosure uses only permitted surfaces. |
...
| `agent-prompt-code-review-part-9-fix-application.md` | `reporting-and-actions.md` — Apply Fixes | Ported explicit opt-in but restricts automatic fixes to `CONFIRMED` findings, adds minimal/low action-only confirmation and editable-target correspondence, requires narrow checks, and preserves the complete survivor pool for backfill. |
