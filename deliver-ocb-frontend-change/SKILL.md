...
5. After Jira verification, use `$plan-mode` and require an approved plan containing the complete `Frontend Workflow Contract` before implementation. Do not create a separate lifecycle tracker.
...
- Use workflow state `WAITING_EXTERNAL` when credentials, permission, approval, tooling, a required design source, or an external system prevents the next required step after safe in-scope recovery is exhausted. When executing through `$execute-plan`, use its technical `Status: Blocked` only when the same condition satisfies that skill's genuine-blocker definition; otherwise remain `MR_PREPARED` and document the exact resume checkpoint.
...
- With `$plan-mode`, place the complete `Frontend Workflow Contract` in the plan and preserve unresolved hard gates as explicit blocking questions.
