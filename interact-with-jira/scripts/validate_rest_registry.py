...
def main() -> int:
    ...
    required_contracts = {
        ...
        "REST API token uses Basic": "Authenticate with HTTP Basic using `email:token`" in rest_workflow,
        "REST API token forbids Bearer": "Never send `JIRA_ACCESS_TOKEN` or any Jira API token with `Authorization: Bearer`" in rest_workflow,
