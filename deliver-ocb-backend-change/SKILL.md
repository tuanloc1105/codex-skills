...
5. After Jira verification, use `$plan` and require an approved plan containing the complete `Backend Workflow Contract` before implementation. Do not create a separate lifecycle tracker.
...
- With `$plan`, place the complete `Backend Workflow Contract` in the plan and preserve unresolved hard gates as explicit blocking questions.
