...
- `kiro-skill/` holds Kiro / Kiro Crew ports of the Codex `discuss`, `plan`, `execute`, `interact-with-git-platform`, `update-agent-docs`, and `simplify` skills, one skill per `kiro-skill/<name>/SKILL.md`. Each keeps the source Codex skill's Markdown-bundle/worktree/review contract but routes questions through `ask_question`, delegation through `spawn_run`, and steering docs through `.kiro/steering/` instead of `AGENTS.md`. Sync each `kiro-skill/<name>/` the same way as a standalone skill (mirror to `~/.codex/skills/kiro-skill/<name>/`); do not flatten it to `~/.codex/skills/<name>/`. To install into a live Kiro Crew instance instead, run `./scripts/sync-kiro-skill.sh` (see [README.md](README.md)), which targets `~/.kiro/crew/skills/kiro-skill/<name>/` and never overwrites KiroCrew's own `~/.kiro/crew/skills/imported/codex/<name>/` mirror of the source Codex skills.
