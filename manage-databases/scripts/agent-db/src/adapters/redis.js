...
import { requireRedisRead } from '../security/safety.js';
import { databaseError, elapsed, verifyIdentity } from './common.js';
...
function withDeadline(client, deadline, task) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new AgentDbError('DATABASE_TIMEOUT', 'Redis operation exceeded its total time budget'));
    }, remaining);
    Promise.resolve()
      .then(task)
      .then((value) => {
...
async function identity(client, target, credential, deadline) {
  ...
  return verifyIdentity(target, {
    ...
    instanceRunId: server.run_id || null,
...
export async function createRedisAdapter(target, credential) {
  const { createClient, RESP_TYPES } = await module();
  ...
  return {
    ...
    async executeMutation() {
      throw new AgentDbError('UNSUPPORTED_OPERATION', 'Redis mutation is not supported in this version');
    },
  };
}
