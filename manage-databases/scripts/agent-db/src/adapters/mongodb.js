...
function options(target, credential, timeoutMs = 15000) {
  return {
...
async function identity(client, target, credential, { timeoutMs = 15000, deadline } = {}) {
  ...
  let serverIdentity = hello.setName || hello.me || null;
  let identityAssurance = hello.setName ? 'replica-set-name' : hello.me ? 'server-reported-address' : 'unverified';
  if (!serverIdentity) {
    try {
...
async function executeMongoMutation(db, target, operation, timeoutMs) {
  if (operation.operation === 'command') {
    invariant((target.allowedNamespaces || []).length === 0, 'NAMESPACE_NOT_ALLOWED', 'Raw MongoDB commands require an unscoped DBA target');
    invariant(operation.command && typeof operation.command === 'object', 'INVALID_ARGUMENT', 'command requires a command object');
    return db.command(operation.command, { maxTimeMS: timeoutMs, timeoutMS: timeoutMs });
  }
  if (operation.operation === 'createCollection') {
    assertAllowedNamespace(target, operation.collection);
    await db.createCollection(operation.collection, { ...(operation.options || {}), maxTimeMS: timeoutMs, timeoutMS: timeoutMs });
    return { created: operation.collection };
  }
  if (operation.operation === 'dropCollection') {
    assertAllowedNamespace(target, operation.collection);
    return { dropped: await db.dropCollection(operation.collection, { maxTimeMS: timeoutMs, timeoutMS: timeoutMs }) };
  }
  ...
  switch (operation.operation) {
    ...
    case 'createIndex': return { index: await collection.createIndex(operation.keys, { ...(operation.options || {}), maxTimeMS: timeoutMs, timeoutMS: timeoutMs }) };
    case 'dropIndex': return { dropped: await collection.dropIndex(operation.name, { ...(operation.options || {}), maxTimeMS: timeoutMs, timeoutMS: timeoutMs }) };
...
export async function createMongoAdapter(target, credential) {
  ...
  const createClient = (timeoutMs = 15000) => new MongoClient(uri, options(target, credential, timeoutMs));
