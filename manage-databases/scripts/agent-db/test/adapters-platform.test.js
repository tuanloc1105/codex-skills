...
const targets = {
  mongodb: {
    connection: { host: '127.0.0.1', port: 27017, database: 'agentdb', authSource: 'admin', tls: false },
  ...
  postgresql: {
    connection: { host: '127.0.0.1', port: 5432, database: 'agentdb', tls: false },
  },
  redis: {
  ...
  for (const [engine, target] of Object.entries(targets)) {
    await t.test(engine, async () => {
      const adapter = await createAdapter(
        ...
        { id: `ci-${engine}-read`, username: engine === 'redis' ? 'default' : 'agentdb', secret: 'unused', mode: 'read' },
      );
