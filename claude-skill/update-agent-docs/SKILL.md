...
4. **Launch multiple agents in parallel.**
   - If subagent tooling is available and permitted by the current session policy, spawn independent agents before writing final docs. In Claude Code, use the `Agent` tool (one call per slice, dispatched in the same message so they run in parallel) when it is callable; otherwise search for the available multi-agent or subagent tool. Use [subagent-prompts.md](references/subagent-prompts.md).
