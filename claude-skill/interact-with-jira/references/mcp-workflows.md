...
Atlassian Rovo MCP is a remote Streamable HTTP server. Before configuration, identify the user's MCP client and inspect its current help/documentation; do not copy Claude Code syntax to another client.
...
/mcp            # then authenticate atlassian through the interactive OAuth flow
```
...
Scope matters: `--scope local` (default) keeps the server in this project for this user, `--scope project` writes it to the repository's checked-in `.mcp.json`, and `--scope user` makes it available across the user's projects. Ask which scope the user wants before writing a shared `.mcp.json`. Equivalent project configuration:
...
Do not overwrite an existing entry. Read the current configuration, preserve unrelated fields, and add or change only values requested by the user. After the change, run:
...
`enabled` and `OAuth` only confirm that the configuration was recognized. Open a new Claude Code session when needed, check `/mcp`, and call exactly one minimal read-only tool to verify that the server connected and OAuth actually works.
...
Before changing anything, inspect the current Claude Code and server entry:
...
2. For an OAuth entry, remove and re-add that exact server using the current CLI syntax and `https://mcp.atlassian.com/v2/mcp` (or `?tools=all` only when a flat tool list is required). Reapply settings that `claude mcp add` does not preserve.
3. Reauthenticate that server through `/mcp` and complete browser sign-in. v2 is a separate OAuth resource, so v1 OAuth credentials do not carry over.
...
6. Verify with `claude mcp list`, `claude mcp get <server-name>`, a new session or `/mcp` when needed, then exactly one minimal read-only identity/resources call. Report success only after the live v2 check passes.
...
- Server is absent: check configuration scope, `claude mcp list`, client restart/new session, and `/mcp`; confirm the URL is the v2 endpoint rather than a retired v1 endpoint.
