...
Claude Code port of the Codex `interact-with-git-platform` skill. The `gh` / `glab` / `tea` contracts are unchanged; run every CLI call through the `Bash` tool and respect this session's git-push protections.
