...
Claude Code port of the Codex `execute` skill. Same bundle-driven execution and evidence contract; delegate phase work with the `Agent` tool, run commands with `Bash`, and persist records with `Write`/`Edit`.
