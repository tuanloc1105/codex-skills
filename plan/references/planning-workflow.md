...
7. Use `request_user_input` to ask the user to approve or revise the plan when the runtime permits approval through that tool; otherwise use the approval route specified by `Question and Open-Issue Contract`. Include its dependency and delegation structure when present. For a permitted tool call, present approval, targeted revision, and broader rework as applicable options; rely on the client-provided free-form path for another choice. Approval is required before changing the existing draft record to its final execute-ready status.
