...
While this structure is active, list both plan references in the Active Snapshot. Remove `references/phase-planning.md` when returning to a simple linear plan, save the updated reference set, and attempt `rules-sync` when supported; acknowledgment does not gate further planning.
...
When phases are useful, assign zero-padded stable IDs such as `P01`, `P02`, and create exactly one self-contained `phases/P<NN>-<slug>.md` file for each phase. Declare every phase file in the `index.md` manifest and link it from the ID/Phase file index in `plan.md`. Record for each phase:
...
Before closing a record write, verify phase IDs and links against the manifest, and validate scheduling metadata in the phase files. Do not duplicate it in new plan tables. If an existing version 4 table duplicates those fields, keep its values equal to the phase files. A declared phase without a file, an unlisted phase file, duplicate phase ID, missing dependency, or dependency cycle blocks approval.
