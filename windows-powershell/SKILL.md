...
- A `foreach (...) { ... }` language statement cannot be followed directly by a pipeline operator. Wrap its output in an array/subexpression first, for example `@(foreach ($item in $items) { $item }) | Format-Table`, or assign the loop output to a variable before piping; otherwise PowerShell reports `An empty pipe element is not allowed`. This also applies inside nested `-Command` payloads: use `$results = @(foreach (...) { ... }); $results | ConvertTo-Json`, never `foreach (...) { ... } | ConvertTo-Json`.
