...
- Freeze the canonical bundle root for the mode lifetime. Tell the user that root and the resume prompt `Use /workflow-discuss and continue the record bundle at <root>`.
...
Resume instruction: Invoke /workflow-discuss, read index.md and the manifest files required by the current state, and continue this exact bundle before substantive work.
...
1. `/workflow-plan` — close this bundle and create a separate plan bundle linked back to it.
2. `/workflow-execute` — make this same bundle execution-ready and let execute adopt it.

For `/workflow-plan`, set `Mode status: Exited`, record the transition in `evidence.md`, close the write transaction, checkpoint, then run `transition plan --record <root>`. `/workflow-plan` creates a distinct bundle under its own saving rules.

For direct `/workflow-execute`, require concrete goal/scope/requirements/constraints, no blocking question, verified baseline and preservation criteria, an executable plan, verification, and initialized evidence/handoff state. Add:
