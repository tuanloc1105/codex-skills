...
13. Use `the required simplification review` to review the complete current-session commit range from the captured starting `HEAD` through the current `HEAD`, together with any remaining in-scope working-tree changes.
14. Fix confirmed or plausible `the required simplification review` findings that are in scope.
...
16. If the current execution session's commit range or remaining working-tree diff contains substantial agent-facing changes that are not already covered in agent docs, use `the repository agent-documentation workflow` with the session-change-only scope in this skill.
