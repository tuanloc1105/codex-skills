...
Guard chỉ tạo plan cho command được classifier nhận là `MUTATION`. Change ID bao phủ cluster ID, environment, Kafka CLI version, exact argv và content hash của mọi input file đã khai báo. Guard từ chối khi file được command tham chiếu chưa được pin bằng `--input-file`, hoặc khi console producer không có payload file.
...
- Input file absolute path, size và SHA-256; với payload, thêm record count.
- Pre-state và kết quả dry-run/preview nếu có.
- Tác động, blast radius, irreversibility, rollback/recovery và post-check.
- Classification action/risk, change ID và confirmation phrase do guard trả về.
...
- `standard`: người dùng gửi đúng `XÁC NHẬN KAFKA <change-id>`.
- `high`: người dùng gửi đúng `XÁC NHẬN KAFKA NGUY HIỂM <cluster-id> <change-id>`.

Chỉ chấp nhận phrase trong tin nhắn mới của chính người dùng sau preview. Một yêu cầu chứa sẵn “tôi xác nhận” vẫn chỉ là yêu cầu mutation ban đầu. Confirmation chỉ dùng một lần cho đúng approval surface và không áp dụng cho thao tác tiếp theo.

## Bước 5: Revalidate và execute một lần

1. Ngay trước execute, chạy classifier và `plan` lại.
2. Yêu cầu classification, risk, cluster ID, environment, exact argv, input hashes và change ID khớp tuyệt đối.
...
Classifier quyết định tier tối thiểu. `high` gồm delete topic/records, topic alter, offset execute/delete, ACL/config/token change, produce, reassignment/verify/cancel, leader election, feature/quorum/broker/storage/process change và các mutation có thể mất dữ liệu hoặc ảnh hưởng cluster-wide. Agent được nâng `standard` thành `high` khi blast radius thực tế lớn hơn, không được hạ tier.
