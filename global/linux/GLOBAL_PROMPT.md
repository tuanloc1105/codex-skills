...
- It is entirely possible to add attribution lines such as `Co-Worker`, `Co-Authored-By`, or similar attribution lines to a commit message.
- Do not use `agent` in branch names. Use a task-focused prefix such as `feature/`, `fix/`, `chore/`, `refactor/`, `docs/`, or `test/`.
