...
metadata:
  ...
  short-description: Author validated technical diagrams as standalone interactive HTML
---

# Technical Diagrams (Kiro / Kiro Crew)

This is the Kiro-native port of the Codex `technical-diagrams` skill. The diagram
engine — the typed JSON schemas, the family renderers, the `node bin/technical-diagrams.mjs`
CLI (`validate` / `deliver` / `visual-check` / `preview` / `import-mermaid` / `brands` /
`guide` / `doctor` / `demo`), the authoring contract, and the viewer runtime — is
agent-agnostic and carries over unchanged. Only the invocation and shell contract are
Kiro-specific:

- Invoke this skill with the `$kiro-technical-diagrams` inline token in chat.
- Run every `node bin/technical-diagrams.mjs …` command with Kiro's `shell` tool, from
  this skill's own directory (`cd` into the directory holding this `SKILL.md`). Node.js
  18+ is required; no install step is needed inside the skill package.
- The delivery, validation, and visual-review contracts below are hard gates: never report
  a non-zero command as success, and never claim a visual inspection you did not perform.
