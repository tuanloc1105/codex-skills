...
test('compare recovery: human diagnostics identify the retained backup and target', { timeout: 70000 }, (t) => {
  ...
  const directories = fs.readdirSync(data.root).filter((name) => name.startsWith('.technical-diagrams-compare-'));
...
function inject(data, failures) {
  ...
  fs.writeFileSync(preload, `
    ...
    const staging = value => inside(value) && path.basename(path.dirname(value)).startsWith('.technical-diagrams-compare-');
...
for (const scenario of cases) {
  test(`compare recovery: ${scenario.name}`, { timeout: 70000 }, (t) => {
    ...
    const staging = fs.readdirSync(data.root).filter((name) => name.startsWith('.technical-diagrams-compare-'));
