...
test('browser renders local-only sources as searchable text and web sources as links', {
  skip: process.env.TECHNICAL_DIAGRAMS_CHROME ? false : 'Set TECHNICAL_DIAGRAMS_CHROME to run evidence browser checks.',
  timeout: 60000,
}, async () => {
  const data = fixture();
...
test('repository evidence is revision-verified, receipt-backed, searchable, and export-clean', () => {
  ...
  assert.match(html, /TechnicalDiagrams\.sourceEvidence = \(function \(\)/);
  ...
  assert.match(html, /TechnicalDiagrams\.sourceEvidence\.installBeacons\(\)/);
