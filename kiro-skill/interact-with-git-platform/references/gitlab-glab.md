...
- On Windows, prefer a UTF-8 file without a BOM over a shell-native multiline argument, because a `cmd.exe` or PowerShell 5.1 pipeline converts non-ASCII characters to `?`. See [windows-unicode.md](windows-unicode.md) before composing the payload.
