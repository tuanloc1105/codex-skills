...
- On Windows, that file must be UTF-8 without a BOM, and non-ASCII text must not travel through an inline `--body`/`--title` argument or an ASCII-defaulted PowerShell pipeline. See [windows-unicode.md](windows-unicode.md) before composing the payload.
