...
- On Windows, prefer a UTF-8 file without a BOM, or a `\uXXXX`-escaped API payload, over a shell-native multiline argument: a `cmd.exe` or PowerShell 5.1 path converts non-ASCII characters to `?`. See [windows-unicode.md](windows-unicode.md) before composing the payload.
