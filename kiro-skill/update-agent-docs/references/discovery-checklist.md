...
Use this checklist to discover the project comprehensively before updating steering docs. Do not paste this checklist verbatim into the steering routing file; convert findings into routing guidance.

## Repository Shape

- Detect repo root, monorepo workspaces, package roots, app roots, library roots, and generated/vendor directories to avoid.
- Read top-level docs: `README*`, `CONTRIBUTING*`, any existing `.kiro/steering/*.md`, root `AGENTS.md`/`CLAUDE.md` (as migration input, not the target), `docs/`, `.github/`, and project-specific planning docs.
...
- Identify the source-of-truth files sessions should read for dependency, framework, runtime, and toolchain versions; do not copy those version numbers into steering docs.
- Identify package boundaries and ownership conventions.

## AI Tooling Docs To Migrate

- Check for agent/tool instructions: `AGENTS.md`, `CLAUDE.md`, `CLAUDE.local.md`, `.claude/rules/`, `.claude/skills/`, `.cursor/rules/`, `.cursorrules`, `.github/copilot-instructions.md`, `.devin/rules/`, `.windsurf/rules/`, `.windsurfrules`, `.clinerules`, `GEMINI.md`, `.gemini/`, `.codex/`.
- Treat these files as evidence, not authority. Migrate durable repo guidance, commands, paths, conventions, and gotchas into `.kiro/steering/`; skip tool-specific syntax that does not apply to Kiro (slash commands, other agents' hook/plugin config).
- Do not copy personal-only preferences, machine-local paths, secrets, or private account details into shared `.kiro/steering/` files — a durable cross-repo personal preference belongs in `learn_add` (with `repo_scope` when it is repo-specific), not in a checked-in steering file.
- If an existing tool config conflicts with the live repo, trust verified repo files and record uncertainty only when it helps future sessions.
...
- For each package/module, identify the first files a session should read for changes in that area.
...
- Identify CI workflows (including any `.github/workflows/*.yml`) and which local checks correspond to CI.
...
- Compare discovered areas with existing `.kiro/steering/*.md` content.
