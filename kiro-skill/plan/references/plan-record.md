...
- With no destination, create `./plans/YYYY-MM-DD-no<N>-<slug>/` relative to the working directory captured at entry (Kiro's `[PROJECT]` active project directory for a dashboard session).
...
- Reserve the selected directory with an atomic create (`write` tool's `create` against a not-yet-existing path). If it already exists because another writer won the race, recompute from the current siblings and retry with the next sequence number. The sequence belongs only to the containing `plans/` directory and is independent of the sequence under `discussion/`.
