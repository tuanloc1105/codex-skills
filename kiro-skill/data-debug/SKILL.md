...
metadata:
  short-description: Read-only DB inspection via the db-debug:latest Docker image
---
...
- Invoke this skill with the `$kiro-data-debug` inline token in chat.
- Run every `docker …` command with Kiro's `shell` tool.
- The read-only default, the mutation boundary, and the secret-handling rules below are
  ...
  URIs, passwords, or tokens in a `shell` command argument or in chat — pass them through
