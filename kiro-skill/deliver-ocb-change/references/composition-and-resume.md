...
The entrypoint's `Question Routing` remains mandatory in standalone and composed flows. A host workflow may further restrict question sequencing and persistence; use its compatible question contract when present, but never weaken the requirement to route the question through the active Kiro surface's interactive selection mechanism or duplicate its question and options in chat text. Reuse exact authorization that remains valid under both contracts instead of asking again. Any new, changed, or retry authorization must go through `Question Routing`.
...
The OCB commit boundary overrides generic incremental-commit cadence. Retain exact pre-source working-branch, LinearB init-commit, and initial-push authorization; create the first-ticket-owned empty init commit before source edits; and satisfy its clean-index, naming, remote publication, and remote-SHA verification requirements. Keep implementation changes uncommitted through verification, stop at `CODE_READY`, and obtain exact post-implementation authorization for the reviewed implementation commit set, required pre-MR empty commit, later push, and MR creation. Authorization for one action does not silently authorize another.
...
5. Reconcile authorization action by action under the unchanged core policy. An existing exact authorization covering the action in this session needs no duplicate confirmation after compaction or a mode handoff. Compaction does not itself start a new session. In a genuinely new session, honor OCB's explicit current-session branch/init-commit/initial-push requirement; older evidence alone does not satisfy it. Do not extend authorization to new repositories, paths, actions, or targets. Post-implementation commit, later-push, and MR authority must still match the reviewed `CODE_READY` proposal.
...
For an in-progress Jira completion checkpoint, restore the exact pre-read and mutation evidence before selecting the next action. If the recorded transition result was rejected or uncertain, first re-read status, Time Spent, and the complete worklog list, then stop automatic mutation: do not retry or switch from the recorded `timeSpent` plus `started` payload, even when the re-read shows no side effects; require new user authorization for any retry. A recorded `started` is evidence only and must never be reused for a new attempt. For every newly authorized attempt, obtain a fresh timestamp immediately before mutation with exactly `date '+%Y-%m-%dT%H:%M:%S.000%z'`, validate the strict Jira format required by core policy, and never reconstruct it from model context during resume.
