...
mr:
  ...
  maximum_changed_line_limit: <positive integer no greater than 155>
jira:
  project_mapping:
...
- `maximum_changed_line_limit` must be positive and no greater than 155. It may make the non-overridable handwritten/non-generated PR-size boundary stricter but cannot exempt agent-authored lines or weaken or broaden the deterministic generated-artifact exception in core policy. Treat the former `advisory_changed_line_limit` key as invalid configuration drift because PR-size excess is not advisory.
...
- A profile cannot silently replace, create, or weaken the default remote Epic base or the rule that it remains every MR target. A verified working branch from the same Epic may be the development base only through the stacked-ticket procedure in core policy; any other exception requires a recorded user override for the exact fallback branch and action. Do not add an owner-role requirement to the Epic base.
