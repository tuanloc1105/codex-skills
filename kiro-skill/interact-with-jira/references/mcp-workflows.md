...
Do not configure the `/v1/sse` endpoint. Do not replace official Atlassian Rovo MCP with a
third-party Jira MCP package unless the user requests it and the risks have been assessed.
...
- Add an MCP server entry pointing at the Streamable HTTP endpoint
  `https://mcp.atlassian.com/v1/mcp/authv2` with OAuth authorization, using this instance's
  own MCP-server configuration mechanism. Describe the action in prose to the user rather
