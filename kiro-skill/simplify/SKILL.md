---
name: kiro-simplify
description: Simplify recently changed code while preserving behavior. Use when the user asks to simplify, clean up, refactor, deduplicate, reduce state or indirection, improve reuse, or make frontend, backend, shared, infrastructure, or test code clearer and smaller in conceptual surface. Kiro port of the Codex `simplify` skill.
metadata:
  short-description: Simplify changed code without behavior drift
---

# Simplify Changed Code (Kiro / Kiro Crew)

This is the Kiro-native port of the Codex `simplify` skill. The four review angles,
target resolution, and behavior-preservation contract are unchanged; independent-review
delegation routes through Kiro Crew's `spawn_run` instead of a Codex-specific subagent
mechanism, and this is the required post-implementation pass invoked by `kiro-execute`.
...
For maintenance comparisons with the Codex `simplify` skill this was ported from, use [upstream-crosswalk.md](references/upstream-crosswalk.md). It is provenance documentation; do not load it during ordinary simplification work.
...
Use independent reviewer-only `spawn_run` passes when the scope is large enough to justify it and session policy permits delegation. Check `resource_status` first for a wide wave. Schedule them within actual concurrency limits; the root (this session) may perform one or more passes while other reviewers work. Do not require a fixed worker count, omit an angle because slots are limited, or claim independent review for a same-context pass. Give each reviewer the same compact scope manifest, changed-file classifications, diffstat, relevant hunks or excerpts, touched symbols, and only its assigned angle — pass all reviewer tasks in one `spawn_run` `tasks` batch, then end the turn and wait for `[Subagent completion event]` messages. Set `include_lessons=true` for a reviewer that may also apply edits later in the same conversation, `include_memory=false` when the task text is fully self-contained. If delegation cannot run (unavailable, refused as back-pressure, or not worth the round-trip for a small diff), perform separate sequential passes locally and disclose that provenance — never a single subagent standing in for "the whole simplify pass."
...
Report applied improvements, skipped decisions, checks run, and meaningful residual risk. If no verified improvement exists, say the scoped code is already sufficiently simple rather than manufacturing cleanup. After any file change, show the required `diff` block per this agent's own output-format rules unless the runtime already renders a structured diff card for that edit.
