...
2. an execution record's explicit session or commit range (e.g. a `kiro-execute` bundle's captured starting `HEAD`);
...
| Pull or merge request | The provider's exact base and head commits, read via `interact-with-git-platform` when available. |
