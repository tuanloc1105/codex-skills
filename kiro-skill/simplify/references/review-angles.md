...
Search changed symbols and adjacent shared modules for existing helpers, validators, parsers, types, constants, policies, components, fixtures, and data-access primitives that could replace new duplication. Use the `code` tool's `search_symbols`/`lookup_symbols` to find candidates rather than guessing from memory.
