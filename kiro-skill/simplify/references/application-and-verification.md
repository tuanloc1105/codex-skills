...
1. Re-read the current file and baseline immediately before editing (with the `write` tool's `strReplace`, which requires the exact current string) so concurrent or user changes are not overwritten.
...
Do not create commits, push, publish, or open a pull or merge request unless the user separately requested that action. When invoked from `kiro-execute`, follow its own incremental-commit cadence instead of committing here directly.
