...
Markdown record-bundle contract, but routes questions through Kiro Crew's `ask_question`
card tool when one is available, and treats Kiro's own memory/ledger tools as
supplementary recovery aids rather than replacements for the bundle.
...
Kiro Crew sessions have a real interactive-question tool. Prefer it over hand-formatted numbered text whenever it is available and the session is not ending anyway:

- **Dashboard session with a decision that blocks further work right now:** call `ask_question` with exactly one question, 2-4 options (mark the recommended one in its `description`), then **end the turn immediately** — the tool is non-blocking and the answer arrives as the user's next message, not as the tool's result. Do not also print the numbered-list format in the same message; the card is the question.
- **Any surface where you are ending your turn anyway, or where `ask_question` is unavailable (Slack thread, CLI, non-dashboard, or the tool errors as not present):** fall back to the numbered chat format below, and/or a trailing `[OPTIONS: ...]` line per this agent's own output-format rules when that is the very last line of the message.
- **Multiple open questions retained in the record but only one presented in chat:** always use the numbered chat format for what is *saved* in `decisions.md`'s `Open Questions`, regardless of which interactive tool presented the most recent one — the Markdown record must stay self-describing for a future session that has no `ask_question` card history.

Numbered chat format contract (used for the saved record always, and for chat whenever `ask_question` is not used):
...
- When an `ask_question` card was used instead of the numbered chat format, still record the same question, its options, and the user's selection into `decisions.md`'s `Open Questions`/`Decisions` using the numbered mapping, so the durable record does not depend on the dashboard's card history.
...
- Before sending a response or saving open questions, check that every question has its own consecutively numbered option list (or, on dashboard, a well-formed `ask_question` card) and that chat has only one pending question. Rewrite any bulleted choices before sending.
...
The user can reply `1` or `4. đánh giá lại phương án fix`. The same choices written with `-` bullets do not satisfy this contract, and neither does an `ask_question` call with more than 4 options or more than one question pending at once during a decision gate.
