...
Use a version 4 Markdown bundle, never a single tracker file. Resolve every relative destination against the working directory captured at skill entry (Kiro's `[PROJECT]` active project directory for a dashboard session) and never ask a storage-choice question.
...
- Reserve the selected directory with an atomic create (`write` tool's `create` command against a path that does not yet exist). If it already exists because another writer won the race, recompute from the current siblings and retry with the next sequence number. The sequence belongs only to the containing `discussion/` directory and is independent of the sequence under `plans/`.
...
- Optionally mirror the canonical bundle root and tracker ID into this session's `session_ledger_record` (`goal`, `next`, `artifacts.bundle_root`) so a mid-compaction wake has a fast pointer back to the bundle before it is reread in full. The ledger is a pointer, never a substitute for reading the bundle.
...
The bundle is authoritative for recorded discussion state, not for live repository, ticket, API, database, or external-system behavior. In `context.md`, classify material claims as verified, user-reported, inferred, proposed, or unknown and include exact locators, revisions or observation times, conflicts, and revalidation conditions. Higher-priority instructions (this agent's own system prompt, safety guardrails, git-push protections) and current live state override stale bundle content.

Use stable decision and question IDs. Preserve superseded history without leaving contradictory entries active. Do not store transcripts, hidden reasoning, secrets, unrelated chat, or raw implementation output. Never store credential values (API keys, tokens, passwords) in any bundle file even when the user pastes them into chat — reference them by name only.
