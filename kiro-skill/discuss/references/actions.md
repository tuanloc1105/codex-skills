...
- Continue to follow all higher-priority safety, approval, and tool constraints — including this agent's safety guardrails, the git-push and destructive-command restrictions, and any per-turn approval prompts.
- Record the authorized scope and results in `actions.md`, with durable evidence in `evidence.md` when needed.

Examples of mutations that may be authorized without leaving the mode include editing non-code documents, creating requested artifacts (including saving a Kiro Crew `artifact_save`/`artifact_update` widget when the user asks for one), changing Figma content, updating tickets or issues (e.g. via `interact-with-git-platform`/Jira tools), sending a requested message (`send_message`), or modifying a specifically named external resource.
...
4. Temporarily lift only the source-code mutation restriction needed for that action. Apply the repository's normal coding, safety, approval, and verification workflows (including any active `surgical-coding`, `code-review`, or language-specific skill); do not broaden the authorization or treat it as permission for unrelated follow-up work.
...
- Inspect the relevant implementation, exports, callers, consumers, routes, schemas, data flows, configs, tests, logs, and docs as needed to understand the current behavior. Use the `code` tool (symbol search, document symbols, pattern search) and `grep`/`glob` for this rather than ad hoc shell scanning. Keep the inspection proportionate to the requested change.
...
- For a genuinely wide baseline sweep (many files, log dumps, broad searches) that would flood this session's context, delegate the sweep to `spawn_run` per this agent's subagent-triggering rules and fold the distilled findings back into `context.md`. Do not spawn a single subagent merely to run the whole discussion for you.
...
- `git push` to a protected branch, any destructive command, or reading a credential file — these remain refused by this agent's floor rules regardless of any scope granted inside discuss.
...
Avoid commands or tools with side effects unless they maintain tracker housekeeping or are necessary for an action authorized under `Scoped Action Authorization` or `Temporary Source-Code Actions`. Before using a mutating tool, verify that its target and effect fit the granted scope. If source-code impact is possible and has not been confirmed, do not run it; disclose the impact and obtain confirmation first. A tool call that this agent's runtime would itself flag as medium/high-risk (installing dependencies, modifying config, anything production-facing) still needs the disclosure called for in the safety guardrails even when scope is otherwise authorized.
