...
- Before creation, inspect Jira create metadata to resolve the exact estimate field and its unit. Convert the chosen hour estimate only through verified Jira semantics. If the project does not expose a writable compatible estimate field, stop and report the limitation instead of omitting or guessing it.
