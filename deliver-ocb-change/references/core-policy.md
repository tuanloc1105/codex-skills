...
An explicit request to create the MR also authorizes one bounded Jira comment on the uniquely resolved working issue after the MR is successfully created and re-read. The comment body must contain only the canonical absolute MR URL. Before commenting, read enough existing comments or issue remote-link evidence to determine whether that exact MR URL is already present; skip when it is. Otherwise add exactly one comment, then re-read and verify the exact URL. If MR creation or its URL remains uncertain, do not comment. If the comment result is uncertain, re-read before any retry and never duplicate a possibly created comment.
...
After the LinearB init commit, record whether the working branch and ticket-owned commits are already observable on the verified GitLab project. Local-only commits preserve local timestamps but are not current remote evidence. Treat missing remote observability as a user-overridable **Hard** gate before implementation: pause, warn that LinearB reporting will remain delayed or incomplete until the post-implementation delivery checkpoint, and continue locally under a scoped override when the user accepts the reporting risk. Never infer push or MR authorization from this override, publish before the checkpoint, or manufacture activity.
...
Jira `Done` and GitLab merge are independent lifecycle facts. Conversely, Jira `Done` never proves pipeline success, Tech Lead approval, merge readiness, or `MERGED`. If permissions, transition metadata, Original Estimate when required, or an applicable Jira transition is unavailable, pause only the Jira transition and continue the MR review-and-merge workflow when its own gates permit.
...
Prepare a complete, reviewable MR with relevant evidence. Require exact, current authorization for working-branch creation, the LinearB init commit, implementation commits, push, and MR creation. An explicit post-implementation request that enumerates the exact implementation commits, pushes, and MRs may authorize those actions together and also authorizes the required pre-MR empty commit described below. Generic approval, plan approval, or authorization for one action does not authorize the others.

Resolve working-branch creation and LinearB init-commit authorization before the first source mutation in every affected repository. This authorization does not cover implementation commits, push, or MR creation. Keep the implementation diff uncommitted while coding and running checks; do not stage unrelated or pre-existing work.

Immediately after the working branch is created and before any source edit, create one LinearB init commit as its first ticket-owned commit. When the working branch already exists, verify that the init commit is already the first ticket-owned commit; never create one retroactively after implementation commits and treat it as valid start evidence. Prefer `git commit --allow-empty` with the required `{jira_id}_{username}_{task_name} chore: initialize LinearB work tracking` message so the timestamp is established without changing delivery files. Verify the index is clean first: never commit, unstage, rewrite, or otherwise incorporate pre-existing or unrelated staged changes. If the index is not clean or an existing branch already contains ticket-owned implementation commits without the init commit, pause before implementation and require the user to resolve the boundary. Record the init commit SHA and timestamp. The init commit is mandatory before `IMPLEMENTING`; it does not authorize or replace the later implementation commit set and does not waive verification.
