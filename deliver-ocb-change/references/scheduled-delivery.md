...
- Obtain separate exact authorization for creating scheduler artifacts, installing the native service, activating it, committing any verified remainder, pushing the exact source branch, and creating the exact MR. The exact MR creation request also authorizes the required pre-MR empty commit under [core-policy.md](core-policy.md); record that action and its exact message separately in the job configuration. Do not infer other actions from one another.
...
9. After any verified remainder commit and before the final push, create and verify the required pre-MR empty commit under [core-policy.md](core-policy.md), recording its SHA, parent SHA, and timestamp atomically. Reuse a verified existing marker on retries and skip marker creation when the exact MR already exists. Push the marker SHA without force and verify the remote source matches. Create the MR without auto-merge, then read it back and verify project, IID/URL, source, target, title, body line breaks, and current SHA. A literal `\\n` in the body is a failure that must be corrected only when exact update authorization exists.
