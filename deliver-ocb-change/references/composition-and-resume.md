...
For an in-progress Jira completion checkpoint, restore the exact pre-read and mutation evidence before selecting the next action. If the recorded transition result was rejected or uncertain, first re-read status, Time Spent, and the complete worklog list, then stop automatic mutation: do not retry or switch from the recorded `timeSpent` plus `started` payload, even when the re-read shows no side effects. Record the timestamp value and whether its provenance was verified actual work start or exactly obtained current local time; never reconstruct it from model context during resume.
