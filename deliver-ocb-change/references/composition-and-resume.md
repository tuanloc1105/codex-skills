...
The entrypoint's `User-Response Contract` remains mandatory in standalone and composed flows. A host workflow may further restrict question sequencing and persistence; use its compatible question contract when present, but never weaken the requirement to invoke `request_user_input` or duplicate the tool question in prose. Reuse exact authorization that remains valid under both contracts instead of asking again. Any new, changed, or retry authorization must use `request_user_input`.
