...
5. After mode, Jira, and Epic-base gates pass or receive valid scoped overrides, assess every intended single-PR slice against the pre-code PR-size gate in the workflow contract. Prefer `$plan` with a complete `OCB Delivery Workflow Contract`. Do not approve the plan or mutate source while any slice estimate is unresolved or exceeds the effective maximum. Instead, recommend a Jira work split and refine it until every independently deliverable slice has a supported compliant estimate, record the slices in the plan, and obtain explicit authorization before creating or editing Jira work items through `$interact-with-jira`. If the user explicitly directs execution without a plan after warning, record the scoped override for skipping the plan only; the PR-size gates remain mandatory. Do not create a separate lifecycle tracker.
...
- Treat every skill-defined gate except the pre-code and actual PR-size gates as warning-first and user-overridable. The PR-size gates cannot be overridden within this workflow. On another gate's failure, pause only the dependent action; state missing evidence, affected action, risk, and recommended fix. If the user explicitly accepts the risk and authorizes the exact action, record the scoped override and continue. Revalidate it after state drift.
...
- Hand off `MR_READY` to Reviewer/Lead ownership. Distinguish verified evidence, accepted assumptions, overrides, and residual risks explicitly.
...
- With `$execute`, keep the approved plan as execution truth, revalidate gates before mutation, obtain OCB branch and local-commit authorization before source mutation, follow `$execute`'s smallest-verified-unit commit cadence, and stop at `MR_READY`, `MR_PREPARED`, or documented `WAITING_EXTERNAL`.
