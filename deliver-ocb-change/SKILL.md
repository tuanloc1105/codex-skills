...
5. After mode, Jira, and Epic-base gates pass or receive valid scoped overrides, prefer `$plan` with a complete `OCB Delivery Workflow Contract`. If the user explicitly directs execution without a plan after warning, record the scoped override. Do not create a separate lifecycle tracker.
