...
6. Resolve branch username from the current request, approved/current plan, or authoritative repository evidence, in that order; never infer it from `git config user.name`. Before the first source mutation, obtain exact current-session authorization for working-branch creation and local incremental commits in every affected repository. Create or verify the working branch from the resolved development base using `{jira_id}_{username}_{task-slug}`. The working branch is always the MR source and the Epic base is always the MR target, including for a stacked ticket; never retarget the MR to the predecessor branch. Keep push and MR creation separately gated unless they were also explicitly authorized.
