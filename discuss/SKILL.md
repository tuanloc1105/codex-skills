...
- Ask exactly one decision question through `request_user_input` under `Question Style`, using its runtime-permitted approval route instead for permission or approval requests. Then end the response and wait for the user's answer.
...
Use `request_user_input` for every user-facing clarification, preference, workflow choice, comprehension check, and free-form value when the tool is available and its runtime contract permits that question. Apply this contract to every reference and companion skill. Do not substitute a prose question for an eligible tool question. Higher-priority tool restrictions take precedence: when the runtime excludes permission or approval requests, use its permitted approval mechanism or a concise final-response question for those requests, never `request_user_input`. Never ask a storage-choice question for the tracker.
