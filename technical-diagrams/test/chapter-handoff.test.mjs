...
test('handoff holds one truthful anchor then settles through one finite camera transaction', () => {
  ...
  assert.match(html, /TechnicalDiagrams\.motionGovernor\.claim\('handoff'/);
  assert.match(html, /TechnicalDiagrams\.motionGovernor\.release\(handoff\.ownerToken\)/);
...
test('latest intent, manual takeover, Still, reduced motion, and hidden pages cleanly settle', () => {
  ...
  assert.match(html, /TechnicalDiagrams\.guidedViews\.cancelHandoff\(reason \|\| 'manual'\)/);
