...
- On adoption, restore the full manifest and applicable references. After compaction, use Mandatory Recovery After Compaction. On later prompts, use `sync_status` to avoid unnecessary rereads. Sync the record and rules when available; acknowledgment is not a prerequisite for delegated implementation.
