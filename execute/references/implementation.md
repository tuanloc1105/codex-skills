...
- Inspect `git worktree list --porcelain`. Reuse a linked worktree only when it is dedicated to this record and has no conflicting unrelated work. Before changing directories on initial execution entry, capture and record the terminal's current working directory as an absolute path. Create new worktrees under `<entry-cwd>/.worktrees/`, including when that directory is a repository subdirectory or outside the affected repository; do not substitute the main repository root. Retain this recorded location on resume so later directory changes do not relocate worktrees or nest new ones inside an implementation worktree. Use distinct child names for different repositories or tasks. If higher-priority repository policy conflicts with this location, report the conflict instead of silently choosing another path. Verify a location inside any Git checkout is ignored by that containing checkout before creating it; use a narrow ignore rule or permitted local Git exclude configuration without hiding unrelated files.
