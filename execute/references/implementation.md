...
- Create each new worktree directly at `<worktree-anchor>/<task-and-repository-name>`, using a unique name for each target repository. For example, an initial terminal directory `/root/git/newfov2` and target repository `/root/git/newfov2/business-services/integration-service` produce `/root/git/newfov2/PAT-2210-integration-service`. Do not substitute `<repository-root>/.worktrees/` or insert a `.worktrees/` parent unless the user explicitly selected that layout. Resolve the destination to an absolute path before running `git -C <source-repository> worktree add <absolute-destination> ...`; `-C` selects the source repository, not the placement anchor. Never overwrite an occupied destination; inspect it for safe reuse or choose a distinct task-specific name under the same anchor.
